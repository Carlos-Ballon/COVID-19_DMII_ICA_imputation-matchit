...
gtsummary::set_gtsummary_theme(my_gtsummary_theme, theme_gtsummary_compact())
...
```{r}
vars_to_keep = c("glucose", "urea", "creatinine") # To impute
...
table_5.1_uv <- data_uv_dm |>
  tbl_uvregression(
    ...
    label = list(
      ...
      neutrofilos ~ "Neutrophils (x10^9/L)",
      creatinine ~ "Creatinine (mg/dL)",
...
table_5.1_mv <-
  coxph(
    Surv(len_hosp_stay, outcome_ph) ~ edad.c + hta + frecuencia_cardiaca + 
      neutrofilos + saturacion_de_oxigeno + creatinine + saturacion_de_oxigeno +
      pao2 + antipaludicos,
    data = data_uv_dm
  ) |>
  tbl_regression(
...
data_imputated_l <- complete(data_imputated, action = "long")
```

```{r}
ggmice(data_imputated, aes(x = .imp, y = urea)) +
...
```{r}
mean_imp <- tapply(data_imputated_l$glucose, 
                  data_imputated_l$.imp, 
                  mean,
                  na.rm = T)

var_mean <- tapply(data_imputated_l$glucose, 
                   data_imputated_l$.imp,
                   var, na.rm = T) / nrow(data_imputated$data)
...
```{r}
par(mfrow=c(1,2))
plot(mean_imp ~ I(1:data_imputated$m),
     ylab = "Mean", xlab = "Imputation",
     main = "Mean\nby imputation")
abline(h=mean(mean_imp), col="red", lty=2, lwd=2)
plot(var_mean ~ I(1:data_imputated$m),
     ylab = "Variance of Mean", xlab = "Imputation",
     main = "Variance of Mean\nby imputation")
abline(h=mean(var_mean), col="red", lty=2, lwd=2)
```


### Multiple data imputation, m = 20, again
...
model_imputation_dm <-
  with(data_imputated_dm, coxph(
    Surv(len_hosp_stay, outcome_ph) ~ edad.c + hta + frecuencia_cardiaca + 
      neutrofilos + creatinine + saturacion_de_oxigeno + pao2 + antipaludicos)
    )
...
tbl_dm_imput <-
  ...
  with(coxph(Surv(len_hosp_stay, outcome_ph) ~ edad.c + 
                                     hta + frecuencia_cardiaca + neutrofilos + 
...
save_as_docx(
  flex_table_2,
  path = stringr::str_glue("Table_2.docx"),
  align = "center")
